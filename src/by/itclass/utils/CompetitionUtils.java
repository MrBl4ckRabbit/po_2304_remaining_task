...
public class CompetitionUtils {
    private static final String PATH_OF_FILE = "src/by/itclass/resources/animals.txt";

    public static void parseFile(List<Cat> cats, List<Dog> dogs, Map<String, String> errors) {
        try (Scanner sc = new Scanner(new FileReader(PATH_OF_FILE))) {
            while (sc.hasNextLine()) {
                fillingCollections(sc.nextLine(), cats, dogs, errors);

            }
        } catch (FileNotFoundException e) {
            System.err.println("File hasn't been found by path " + PATH_OF_FILE);
            System.exit(1);
        }
    }
    ...
    private static void processException(Map<String, String> errors, CompetitionException e) {
        ...
        if (matcher.find()) {
            errors.put(matcher.group(), String.format("Error in line \"%s\"-%s", e.getErrorLine(), e.getCause(), e.getMessage()));
        }
    }
